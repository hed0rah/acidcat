...
BODY = """<div class="sheet">
  ...
  <div class="sec">file regions</div>
    ...
    <details class="region">
      <summary><span class="chev">&#9656;</span><span class="rname">Sound Tracker (STC)</span><span class="rspan">the ancestor . fixed blocks</span></summary>
      <div class="rbody">
        <p class="note">STC is the Sound Tracker 1.x module as its Song Compiler wrote it, the
        format PT2 and PT3 grew out of. No magic. The header is 27 bytes of pointers and the four
        blocks come in a fixed order with fixed record sizes, so the file tiles by arithmetic up to
        the pattern table and by pointers after it:</p>
        <div class="kv">
          <div><span class="k">0x00</span><span class="v">delay; then pointers to the positions block, the ornaments and the pattern table</span></div>
          <div><span class="k">0x07</span><span class="v">18-character identifier, usually <code>SONG BY ST COMPILE</code>; then the module's size</span></div>
          <div><span class="k">0x1B</span><span class="v">samples, 99 bytes each: number, 32 rows of 3 bytes, repeat position, repeat length</span></div>
          <div><span class="k">positions</span><span class="v">count &minus; 1, then (pattern, transposition) pairs, ending exactly at the ornaments</span></div>
          <div><span class="k">ornaments</span><span class="v">33 bytes each: number, 32 semitone offsets</span></div>
...
BUILDS = """
  ...
  build("pt3-head","byte",
    ...
    [
      ...
      {label:"delay",k:"enum",r:[1,1],val:"5",
       ...
       note:"0x64."},
      {label:"positions",k:"sync",r:[2,2],val:"7",
  ...
  build("pt3-smp","byte",
    ...
    [
      {label:"loop",k:"enum",r:[0,0],val:"0",
       ...
       note:""},
      {label:"length",k:"sync",r:[1,1],val:"4 rows",
  ...
  build("pt3-orn","byte",
    ...
    [
      ...
      {label:"length",k:"sync",r:[1,1],val:"1",body:"One offset follows.",note:""},
