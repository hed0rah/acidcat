...
v1.4.1 · ~53k source LOC · ~46k test LOC · one hard dependency (`mutagen`);
...
1. **Format primitives** -- `core/formats/` (per-format byte decoders: `riff`,
   ...
   `core/infra/` (`sniff.py` -- 73 recognized formats, `fieldcodec.py` -- the
   enc-language, `geometry.py` -- which bytes a chunk occupies, `mapped.py`,
   `render.py`).
2. **Walkers** -- `core/walk/*.py`: 43 walkers behind one dispatcher, serving 69
...
src/acidcat/
  core/            165 modules
    ...
    walk/          43 walker modules -> 69 format labels (44)
  ...
  cli.py  explorer.py  tui_theme.py  __init__.py     (222 modules in total)
