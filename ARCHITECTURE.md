...
1. **Format primitives** -- `core/formats/` (per-format byte decoders: `riff`,
   ...
   `core/infra/` (`sniff.py` -- 88 recognized formats, `fieldcodec.py` -- the
   enc-language, `geometry.py` -- which bytes a chunk occupies, `mapped.py`,
   `render.py`).
2. **Walkers** -- `core/walk/*.py`: 53 walkers behind one dispatcher, serving 84
...
src/acidcat/
  core/            186 modules
    formats/       per-format byte decoders (31)
    walk/          53 walker modules -> 84 format labels (54)
  ...
  cli.py  explorer.py  tui_theme.py  __init__.py     (243 modules in total)
lab/src/acidcat_lab/
