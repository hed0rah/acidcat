...
v1.6.0 · ~56k source LOC · ~52k test LOC · one hard dependency (`mutagen`);
...
1. **Format primitives** -- `core/formats/` (per-format byte decoders: `riff`,
   ...
   `core/infra/` (`sniff.py` -- 74 recognized formats, `fieldcodec.py` -- the
   enc-language, `geometry.py` -- which bytes a chunk occupies, `mapped.py`,
   `render.py`).
2. **Walkers** -- `core/walk/*.py`: 45 walkers behind one dispatcher, serving 70
...
src/acidcat/
  core/            168 modules
    formats/       per-format byte decoders (22)
    walk/          45 walker modules -> 70 format labels (46)
  ...
  cli.py  explorer.py  tui_theme.py  __init__.py     (225 modules in total)
lab/src/acidcat_lab/
