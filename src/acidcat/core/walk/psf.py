...
def inspect_psf(filepath, deep=False):
    ...
    if h["reserved_size"]:
        chunks.append(_region("reserved", psfmod.HEADER, h["reserved_size"],
                              "%d bytes, platform-specific" % h["reserved_size"]))
    ...
    if h["inflated_size"] is None:
        warns.append("the program does not inflate as zlib")
    if h["gsf"] and not h["gsf"]["consistent"]:
        warns.append("the GBA header declares %d bytes of ROM and %d follow"
...
def _program_chunk(h, short, is_lib):
    ...
    if g:
        fields += [
            _f(None, 0, "entry_point", "0x%08X" % g["entry"],
        ...
        if g["length"] <= 4 and not is_lib:
            ...
            summary = "%s library: a {:,}-byte GBA ROM".format(g["length"]) % short
        else:
