...
| `acidcat probe FILE read\|scan\|find\|strings\|hexdump\|diff ...` | Low-level byte dissection (RE-tool surface): typed read at an offset (`read fmt.sample_rate -t u32`), value scan, byte-pattern find, strings, hexdump, and diff. Addresses can be raw offsets or structural names (`chunk` / `chunk.field`) resolved through the walker |
